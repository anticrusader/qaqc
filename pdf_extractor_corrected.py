...
def extract_pdf_info_corrected(pdf_path):
    """
    Corrected version that extracts 7 fields:
    1. Drawing Title
    2. Drawing Number  
    3. Revision
    ...
    7. Table Title (e.g., "Design Development")
    """
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for i, line in enumerate(lines):
                ...
                if 'Rev.' in line and 'Date' in line and 'Reason For Issue' in line and 'Chk' in line:
                    ...
                    # Look for table title below the header
                    for j in range(i + 1, min(i + 5, len(lines))):
                        title_line = lines[j].strip()
                        if title_line and not any(keyword in title_line for keyword in ['Project', 'Drawing', 'Model', 'Drawn', 'Key Plan']):
                            if len(title_line.split()) <= 3:  # Table titles are usually short
                                table_title = title_line
                                break
                    
                    # Found the header, now look for revision entries above it
                    # Revision entries are typically above the header
                    for j in range(max(0, i-15), i):
                        entry_line = lines[j].strip()
                        
                        # Look for revision entry pattern with more flexible regex
                        # Pattern: REV DATE REASON... CHK
                        revision_entry_pattern = r'^(\d{2}|[A-Z]\d+)\s+(\d{2}/\d{2}/\d{4})\s+(.+?)\s+([A-Z]{1,3})$'
                        match = re.match(revision_entry_pattern, entry_line)
...
def process_all_pdfs_corrected(directory_path="."):
    """
    Process all PDF files using the corrected extraction method.
    """
    ...
    for pdf_file in pdf_files:
        ...
        result = extract_pdf_info_corrected(str(pdf_file))
    ...
    # Display results
    print("\n" + "="*150)
    print("CORRECTED EXTRACTION RESULTS:")
    print("="*150)
...
if __name__ == "__main__":
    ...
    results = process_all_pdfs_corrected()